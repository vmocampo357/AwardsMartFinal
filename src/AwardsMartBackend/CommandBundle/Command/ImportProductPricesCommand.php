<?php

namespace AwardsMartBackend\CommandBundle\Command;

use Symfony\Bundle\FrameworkBundle\Command\ContainerAwareCommand;
use Symfony\Component\Console\Input\InputInterface;
use Symfony\Component\Console\Output\OutputInterface;
use PDO;

class ImportProductPricesCommand extends ContainerAwareCommand
{
    /**
     * {@inheritdoc}
     */
    protected function configure()
    {
        $this
            ->setName('aw:import_product_prices_command')
            ->setDescription('Hello PhpStorm');
    }

    /**
     * {@inheritdoc}
     */
    protected function execute(InputInterface $input, OutputInterface $output)
    {

        ini_set('memory_limit','1024M');

        $query = "
            SELECT A.Size_Label, A.SizeCategory, P.Min, P.Max, Is_Active, ProductID, Price FROM awards_prices P
            INNER JOIN awards_sizes A
            ON P.Size_Id = A.Size_Id
            ORDER BY ProductID DESC";

        /*
         * 1) OK. Let's start by seeing if we can't connect to the OLD database,
         * which we'll hardcode the information for, and see if we can't make a big ol
         * query on the old stuff. If not, we'll have to use what we already have.
         */
        $pdo = new PDO("mysql:host=localhost;dbname=awardsmart_old_better",'root','');

        /*
         * 2) If we get a chance to connect, we can go ahead and make the first query, which
         * is to select all CATEGORIES, and find out how many need to process.
         */
        $results = $pdo->query($query);

        $output->writeln([
            "==========",
            sprintf("Found %d results in the OLD database",$results->rowCount()),
            "=========="
        ]);

        /*
         * 3) Go through each CATEGORY returned by the result set, then, using the entity
         * manager, start creating the different CATEGORY entities, persisting, and then
         * flushing.
         */
        $new_pdo = $this->getContainer()->get('doctrine.dbal.default_connection');

        $secondary = [];

        // Start by building a structured insert array
        $output->writeln("Setting initial inserts...");
        foreach($results->fetchAll(PDO::FETCH_OBJ) as $product_size_price){

            // Setting the SIZES
            $secondary
                [$product_size_price->ProductID]
                [(string)$product_size_price->Size_Label]['prices'][] = array(
                "min"       => $product_size_price->Min,
                "max"       => $product_size_price->Max,
                "price"     => $product_size_price->Price);

            // Setting the CATEGORY label
            $secondary
                [$product_size_price->ProductID]
                [(string)$product_size_price->Size_Label]['category']
            = $product_size_price->SizeCategory;
        }

        // Then, insert according to what we already know
        $tertiary = [];
        $output->writeln("Inserting secondary records...");
        foreach($secondary as $product => $sizes){
            foreach($sizes as $size => $info){
                $new_pdo->insert("product_size",array(
                    "size" => $size,
                    "units" => $info['category'],
                    "product_id" => $product
                ));
                $last_insert = $new_pdo->lastInsertId();
                $tertiary[$last_insert] = $info['prices'];
            }
        }

        // Finally, insert prices based on the inserted sizes
        $output->writeln("Inserting tertiary records...");
        foreach($tertiary as $insert_id => $insert){
            foreach($insert as $in){
                if(@$in['price'] != 0){
                    $new_pdo->insert("size_price",array(
                        "size_id" => $insert_id,
                        "min" => @$in['min'],
                        "max" => @$in['max'],
                        "price" => floatval(@$in['price']),
                        "is_single" => 0
                    ));
                }
            }
        }

        $output->write("Finished! ...");

    }
}
