...
class ImportProductOptionsCommand extends ContainerAwareCommand
{
    /**
     * {@inheritdoc}
     */
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $query = "SELECT * FROM awards_colors";
        ...
        foreach($results->fetchAll(PDO::FETCH_OBJ) as $option){
            $new_pdo->insert("product_option",array(
                "id" => $option->Color_Id,
                "code" => $option->Color_Code,
                "name" => $option->Color_Name,
                "is_active" => ( $option->Is_Active == "Y" ) ? 1 : 0
            ));
        }
